...
const skipOnboarding = async () => {
    const textView = await (notesPage.textView);
    await expect(textView).toHaveText("Welcome to ColorNote");
    
    // Skip Tour Notepad
    const button = await (notesPage.button);
    await expect(button).toHaveText("SKIP");
    await button.click();

    // Assert button Add
    await expect(notesPage.btnAdd).toBeDisplayed();
}
...
const createTextChecklist = async () => {
...
const createTextMultipleChecklist = async () => {
        const indexToClick = 1;
        let randomItem;
        const selectedItems = [];
